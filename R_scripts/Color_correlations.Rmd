...
lum <- avg_img %>% 
  select(ID, pl_code, dblMean, dblSD, uvMean, uvSD, area, Age, pop, lat, lon, Year, prepartor) %>% 
...
z <- lmerTest::lmer(dblMean_c ~ dblMean_d + (dblMean_d|Year) + (dblMean_d|prepartor), data = lum_wide, na.action = na.exclude) 
summary(z)
